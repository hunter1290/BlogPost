...
function Home() {
 ...
 const getBlog = async()=>{
   const data = await fetch('https://blog-post-liart-two.vercel.app/get-blog');
       ...
       <Container>
        ...
        <TopBar>
         ...
         <End>
          {
            blog&&blog.map((x,key)=>(
             <BlogCard key={key} title={x.title} likes = {x.likes} image = {x.image} author={x.author} content={x.content} id={x._id}/>
            ))
          }
         </End>
